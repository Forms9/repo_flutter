...
class SaleBillItemModel {
  ...
  @override
  String toString() {
    return "$barcodeNo,$productName,$quantity,$rate,$disRate,$itemTotal";
  }
  ...
  String getIndex(int index) {
    switch (index) {
      ...
      case 4:
        if (rate! >= 0) {
          return "£${disRate!.toStringAsFixed(2)}";
        } else {
          return "£${rate!.toStringAsFixed(2)}";
        }
      case 5:
...
class TabletGenerateBillPage extends StatefulWidget {
  ...
  @override
  State<TabletGenerateBillPage> createState() => _TabletGenerateBillPageState();
}

class _TabletGenerateBillPageState extends State<TabletGenerateBillPage> {
  String action = "";
...
class _BillPageState extends State<BillPage> {
    ...
    setState(() {
      if (!saleMode) {
        ...
        if (saleController.text.isNotEmpty) {
          if (double.parse(saleController.text) > 0) {
            discountTotal =
                cartTotal * (1 - (double.parse(saleController.text) / 100));
          } else {
            discountTotal = cartTotal;
          }
        }
  ...
  saveBillState() async {
    try {
      ...
      if (saleMode) {
        //SALE MODE BILL
        List<SaleBillItemModel> newTemp = List.empty(growable: true);
        for (BillItemModel item in temp) {
          newTemp.add(
            SaleBillItemModel(
              indexNo: item.indexNo,
              barcodeNo: item.barcodeNo,
              ...
              rate: item.rate,
              disRate:
                  item.rate! * (1 - (double.parse(saleController.text) / 100)),
              itemTotal: item.quantity! *
                  item.rate! *
                  (1 - (double.parse(saleController.text) / 100)),
            ),
          );
        }
        // ignore: unused_local_variable
        ...
        var lists = [];
        int chunkSize = 10;
        for (var i = 0; i < newTemp.length; i += chunkSize) {
          lists.add(newTemp.sublist(i,
              i + chunkSize > newTemp.length ? newTemp.length : i + chunkSize));
        }
                            ...
                            children: [
                              ...
                              pw.Table.fromTextArray(
                                ...
                                headers: List<String>.generate(
                                  saleTableHeaders.length,
                                  (col) => saleTableHeaders[col],
                                ),
                                    ...
                                    children: [
                                      ...
                                      if ((cartTotal - discountTotal) != 0)
                                        pw.Padding(
                                          ...
                                          child: pw.Text(
                                              'Discount : - £${(cartTotal - discountTotal).toStringAsFixed(2)}',
                                              style: const pw.TextStyle(
                                                  fontSize: 8)),
                                        ),
                                    ]),
              ...
              child: ListView(
                ...
                children: [
                  ...
                  Visibility(
                    ...
                    child: Padding(
                      padding: const EdgeInsets.only(
                          top: 5, left: 16, right: 16, bottom: 2),
                      child: Row(
                        children: [
                          // Expanded(
                          //   child: Text(
                  ...
                  const Padding(
                    padding:
                        EdgeInsets.only(top: 5, bottom: 0, right: 13, left: 13),
